create table t_building (
  ...
  name                          varchar(255) comment '楼栋编号，罗马数字',
...
create table t_catch_photo (
  ...
  card_id                       bigint comment '物理卡，卡号开门',
  user_id                       bigint comment '用户，手机开门',
  user_info_id                  bigint comment '住户，手机开门',
  create_time                   timestamp DEFAULT CURRENT_TIMESTAMP not null comment '创建时间',
  update_time                   timestamp DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP not null comment '修改时间',
...
create table t_complain (
  ...
  building_id                   bigint comment '所属楼栋编号',
  unit_id                       bigint comment '所属单元编号',
  user_info_id                  bigint not null comment '住户',
  ...
  constraint uq_t_complain_user_info_id unique (user_info_id),
...
create table t_open_door_log (
  ...
  card_id                       bigint comment '物理卡，卡号开门',
  user_id                       bigint comment '用户，手机开门',
  user_info_id                  bigint comment '用户，手机开门',
...
create table t_repair (
  ...
  constraint uq_t_repair_user_info_id unique (user_info_id),
...
create table t_sms (
  ...
  building_id                   bigint comment '所属楼栋编号',
  unit_id                       bigint comment '所属单元编号',
  target                        varchar(1000) comment '发送目标',
...
create table t_user_info (
  ...
  permission                    varchar(1000) comment '开门权限',
...
alter table t_complain add constraint fk_t_complain_user_info_id foreign key (user_info_id) references t_user_info (id) on delete restrict on update restrict;

alter table t_repair add constraint fk_t_repair_user_info_id foreign key (user_info_id) references t_user_info (id) on delete restrict on update restrict;
