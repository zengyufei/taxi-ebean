...
public abstract class AbstractRoomController<T extends AbstractIdEntity, S extends AbstractIdEntity> extends AbstractController<T> {
	...
	public Msg queryByRoomId(@Min(value = 1, message = "查询的 id 不能为空")
	                              @NotNull(message = "id 不能为空")
			                              Long id) {
		List<T> result = Ebean.find(entityClass)
				.where()
