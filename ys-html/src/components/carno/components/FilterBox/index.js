    ...
    render() {
        ...
        return (
            <div className={styles.boxPadding}>
                {isSmall && <div style={{ marginBottom: '10px' }}>{btns}</div>}
                {!isSmall && <div style={{ marginBottom: '10px' }}>{btns}</div>}
                <div className={btnsCls}>
                    {newFields && (
                        <Button
                            className={filterBtnCls}
                            onClick={() => this.handleFilterClick(visible)}
                            style={{ float: 'left' }}
                        >
                            过滤<Icon type="down"/>
                        </Button>
                    ...
                    {searchCacheKey && (
                        <Popover
                            ...
                            content={
                                <CheckboxGroup
                                    defaultValue={newFields.map(e => e.key)}
                                    onChange={e => this.handleChangeFilter(e, searchCacheKey, searchFields)}
                                >
                                    <Row style={{ width: formProps.width || '150px' }}>
                                        {searchFields.map(e => {
                                            return (
                                                <Col key={e.key} style={{ margin: '3px' }} span={24}>
                        ...
                        >
                            ...
                            >
                                过滤条件<Icon type={filterIcon ? 'down' : 'up'}/>
                            </Button>
                        </Popover>
                    )}
                </div>
