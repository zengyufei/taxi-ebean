 ...
 */
function getFieldValue(value, field = {}, record, fn) {
    ...
    return fieldTypes[type](value, field, record, fn)
}
 ...
 */
function getColumns(fields, fieldKeys, extraFields, fn) {
    const chain = {}
    let columns = []
    ...
    const transform = _fields => {
        return _fields.map(field => {
            ...
            if (!render) {
                render = (value, record) => {
                    return getFieldValue(value, field, record, fn)
                }
            }
                ...
                if (fieldKey === (item.key || item.dataIndex)) {
                    ...
                    column = {
                        ...
                        render: hasRender
                            ? item.render
                            : (value, record) => {
                                return getFieldValue(value, item, record, fn)
                            },
                    }
                }
            }
    ...
    const enhance = _extraColumns => {
        ...
        _extraColumns.forEach(extraColumn => {
            ...
            for (let i in columns) {
                ...
                if (item === undefined) {
                    column = extraColumn.dataIndex
                } else if (item.dataIndex === extraColumn.dataIndex) {
