...
const getAllBuilding = `/building/queryAll`
const queryByCommunityIdUrl = `/building/queryByCommunityId`
...
export default extend({
  ...
  effects: {
    ...
    * queryPage(payload, { get, getMessage, update, formBindType, syncGet }) {
      ...
      yield formBindType({
        ...
        buildingId: ({ initialValue, meta, field, inputProps, placeholder, isText, currentForm }) => {
          ...
          let options = []
          if (isOpten) {
            let allBuilding = syncGet(queryByCommunityIdUrl, { id: currentForm.getFieldValue('communityId') })
