...
const resourceMapByparentIdKey = 'resourceMapByparentId'
...
export default extend({
  ...
  state: {
    ...
    [resourceMapByparentIdKey]: local.get(resourceMapByparentIdKey) || [],
  ...
  effects: {
    ...
    * queryResources({}, { get, update, arrayToTree, localCache, select, formBindType, _ }) {
      let { resource, resourceTree, resourceSecondTree, resourceMap, resourceMapByparentId } = yield select(({ sysStore }) => sysStore)
      ...
      if (!resource || resource.length === 0) {
        ...
        if (!resource) {
          ...
          _.cloneDeep(result).forEach(e => {
            ...
            if (resourceMapByparentId[e.parentId]) {
              resourceMapByparentId[e.parentId].push(e)
            } else {
              resourceMapByparentId[e.parentId] = [e]
            }
          })
          resourceSecondTree = yield arrayToTree(resourceSecondArray)
          resourceTree = yield arrayToTree(result)

          yield update({ [resourceKey]: result, resourceTree, resourceSecondTree, resourceMap, resourceMapByparentId })
          ...
          localCache.set(resourceMapByparentIdKey, resourceMapByparentId, 120)
        }

        yield formBindType({
          parentId({ initialValue, field }) {
            const loop = treeList => treeList.map(treeNode => {
              if (treeNode.children) {
                return (
                  <TreeNode key={treeNode.id} value={treeNode.id} title={treeNode.name} item={treeNode}>
                    {loop(treeNode.children)}
                  </TreeNode>
                )
              }
              return <TreeNode key={treeNode.id} value={treeNode.id} title={treeNode.name} item={treeNode} />
            })
            let result = {
              input: (
                <TreeSelect
                  key="parentIdSelectForm"
                  allowClear
