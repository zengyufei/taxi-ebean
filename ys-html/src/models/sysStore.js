    ...
    effects: {
        * queryOrgs({}, { get, update, localCache, arrayToTree, arrayToMap, tableBindType, formBindType }) {
            ...
            yield tableBindType({
                parentOrgNo: value => {
                    if (value) {
                        return orgMap[value] && orgMap[value].orgName
                    }
                    return '顶级组织'
                },
            })
            ...
            yield formBindType({
                ...
                parentOrgNo: ({ initialValue, field }) => {
                    const loop = treeList =>
                        treeList.map(treeNode => {
                            if (treeNode.children) {
                                return (
                                    <TreeNode
                                        key={treeNode.orgNo}
                                        value={treeNode.orgNo}
        ...
        * queryRoles({}, { get, update, localCache, arrayToMap, tableBindType, formBindType }) {
            ...
            yield tableBindType({
                roleId: value => {
                    ...
                    if (value) {
                        return roleMap[value] && roleMap[value].roleName
                    }
                    return ''
                },
            })
                    ...
                    let result = {
                        input: (
                            <Select key="roleIdSelectForm" allowClear placeholder={placeholder || '请选择角色'}>
                                {options.map(item => (
                                    <Option key={`roleId${item.key}`} value={item.key}>
                                        {item.value}
                                    </Option>
                                ))}
                            </Select>
        ...
        * queryResources({}, { get, update, arrayToTree, localCache, select, formBindType, _ }) {
            let { resource, resourceTree, resourceSecondTree, resourceMap, resourceMapByparentId } = yield select(
                ({ sysStore }) => sysStore,
            )
