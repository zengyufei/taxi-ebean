...
import {
  prefix, name, storeName, defaultTableFields, // 表格
} from './constant'
...
const List = options => {
  ...
  const { openAddPage, openUpdatePage, onShowSizeChange, onChange, handlerDelete } = methods
  ...
  return (
    <div>
      <div style={{ padding: 10 }}>
        <Button type="primary" icon="plus-circle-o" onClick={openAddPage}>新增{name}</Button>
      </div>
      <Table
        scroll={{ x: 1200 }}
        rowKey="id"
